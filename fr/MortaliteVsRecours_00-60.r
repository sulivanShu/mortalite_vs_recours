...
EvoMortStd = 
	...
	subset(FREQ == "A" & INDICATEUR == "TAUX_MORTALITE_STANDARDISE" & grepl("^D", REF_AREA) & AGE == "00-64") |> 
...
# Nombre de personnes de moins de 60 ans par département fois 100
PondPop60 = get_idbank_list("TCRED-ESTIMATIONS-POPULATION") |> 
	...
	unlist() |> 
	as.numeric() |>
	(\(data) { sapply(seq(1, length(data), by = 2), \(start) { (100-(data[start])) * data[start+1] }) })() |>
	print()
...
png("EvoMortStdVsEvoTxPat-65Ans.png")
